...
def get_iterator(board):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    return iter(sorted(board.items()))
...
def print_board(board):
    ...
    for i in range (n):
        for j in range (n):
            if board[i,j]: print (chr(11035), end='')
            else: print (chr(11036), end='')
        print() 
...
def neighbors(p):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    return {(p[0], p[1]+1), (p[0], p[1]-1), (p[0]-1, p[1]-1), (p[0]-1, p[1]+1), (p[0]+1, p[1]+1), (p[0]-1, p[1]), (p[0]+1, p[1]), (p[0]+1, p[1]-1)}
...
def place_blinker(board, p = (0,0)):
    ...
    while n >= 0: 
        for i in range (1,3):
            set_alive(board, (p[0]+i, p[1]), True)
        n -= 1
...
def tick(board):
    ...
    """
    """GRAPSTE TON KWDIKA SAS EDW."""
    new_gen = copy_board(board) # giati otan kanoume elegxo oi allages prepei na apothikeuontai sto new_gen wste na min epireastei o board
    for tup in board:
        discarded=[] # lista pou periexei osous neighbors prepei na afairesoume 
        a=neighbors(tup)
        for item in a:    
            if item not in board:
                discarded.append(item)
        for item_2 in discarded:
            a.discard(item_2)
        if is_alive(board, tup):
            sum1 = 0
            for item in a:
                if is_alive(board, item):
                    sum1 += 1
            if sum1 == 1 or sum1  == 0 or sum1 >= 4:
                set_alive(new_gen, tup, False)
        else:
            sum2 = 0
            for item in a:
                if is_alive(board, item):
