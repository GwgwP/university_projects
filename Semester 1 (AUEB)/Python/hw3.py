...
def location(name, lat, lon, type):
    ...
    brisketai sto gt kai lon antistoixaewgrafiko platos kai mikos la. To type
    einai string pou perigrafei to eidos tis topo8esias, p.x., 'monument',
    'bus station'.
    """
    return [f"{name}", lat, lon, f"{type}"]
...
def print_location(loc):
    ...
    print(f"{name(loc)} ({type(loc)}) at coordinates {lattitude(loc)}, {longitude(loc)}")
...
def nearest_location(loc, loc_list, loc_type=None):
    ...
    """
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    distances=[]   # arxikopoioume mia lista pou tha exei oles tis apostaseis apo to loc
    i = 0
    while i < len(loc_list): #vriskoume tis apostaseis tou loc apo kathe stoixeio tis dosmenis listas kai to ekxwre stin distances
        if loc_type == None:
            if loc != loc_list[i]:
                distances.append(distance(loc, loc_list[i]))
        else:
    ...
    flag = True 
    i = 0 
    while flag and i < len(loc_list):
        if m == distance(loc, loc_list[i]):
            thesi_min = i
            flag = False
        i += 1

    return (loc_list[thesi_min])  
#--------------------- Askisi 3 -----------------------
...
def flatten(field):
    ...
    """
    """GRAPSTE TON KWDIKA SAS APO KATW."""
    new_list=[] #i nea zitoymeni lista 
    temp=field #Gia na min tropopoih8ei h field 
    flag = True
    while flag:
        if temp[1] == None:
            flag = False
...
def cherry_string(field):
    ...
    return reduce(add, [item for item in flatten(field) if 'cherry' in item])
